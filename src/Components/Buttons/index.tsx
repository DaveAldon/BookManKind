...
export function DeactivatedButton(props: IButton) {
  const { onPress, style } = props;
  return (
    <TouchableOpacity onPress={onPress} style={[{ backgroundColor: GlobalStyles.Colors.backgrounds.DARKEST, height: 50, borderRadius: 10 }, style]}>
      <View style={{ justifyContent: "center", alignItems: "center", height: "100%" }}>{props.children}</View>
    </TouchableOpacity>
  );
}
